...
+ (id)sharedManager {
    static StringEncryption *sharedMyManager = nil;
...
- (NSData *)encrypt:(NSData *)plainText key:(NSString *)key  iv:(NSString *)iv {
    ...
    if(patchNeeded)
    {
        DLog(@"Key length is longer %lu", (unsigned long)[[[StringEncryption alloc] md5:key] length]);
...
- (NSString *) encryptPlainTextWithData:(NSData *)plainData key:(NSString *)key iv:(NSString *)iv {
    return [[[[StringEncryption alloc] init] encrypt:plainData key:key iv:iv] base64EncodedStringWithOptions:0];
}

- (NSData *) decryptCipherTextWith:(NSString *)ciperText key:(NSString *)key iv:(NSString *)iv {
    return [[StringEncryption alloc] decrypt:[[NSData alloc] initWithBase64EncodedString:ciperText options:NSDataBase64DecodingIgnoreUnknownCharacters] key:key iv:iv];
}
